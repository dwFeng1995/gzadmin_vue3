...
const wetchPublicFileRef = ref(null)
...
const state = reactive({
    ...
    detail: {},
    companyId: '',
    uploadWetchFileDialogVisible: false,
    OrderIndexArr: [],
    company_noArr: [],
})
...
const createDialogClosed = () => {
    ...
    console.log(createChildRef.value, 'createChildRef');
}
const submitCreate = () => {
...
const refreshData = (flag) => {
    ...
    state.uploadWetchFileDialogVisible = false
    if (flag) {
        getMchPlatformList()
    }
}
...
const getMchPlatformDetail = async ({ id }) => {
    const res = await getMchPlatformDetailApi(id, { pay_type_code: 'W' })
...
const uploadWetchFileDialogClosed = () => {
    wetchPublicFileRef.value.resterForm()
}

const submitUpload = () => {
    wetchPublicFileRef.value.submitForm()
}
