<template>
    <div>
        <el-card class="box-card" :body-style="{padding: 16}" shadow="never">
            ...
            <ul>
                <li v-for="item in tableData" :key="item.id">
                    <p class="flex-row justify-between  parent-item">
                        <span>
                            {{item.title}}   <a href="javascript:;"  class="path ml-16">{{item.name}}</a>
                        </span>
                        <span>
                            <span class="mr-16">排序: {{item.order}}</span>
                             <a @click="getAdminMenuDetail(item)" class="edit-icon mr-12" href="javascript:void(0)"><i class="el-icon-edit"></i></a>
                              <a @click="deleteAdminMenu(item)" class="delete-icon" title="标题" href="javascript:void(0)"><i class="el-icon-delete"></i></a>
                        </span>
                    ...
                    <ul class="ml-50" v-if="item.child.length >0">
                        <li class="child-item" v-for="child in item.child" :key="child.id">
                            <span class="flex-row justify-between">
                                <span>
                                    {{child.title}}
<!--                                    <router-link v-if="child.uri" class="path" :to="child.uri">{{hostName}}{{child.uri}}</router-link>-->
                                     <a href="javascript:;"  class="path ml-16">{{child.name}}</a>
                                </span>
                                <span>
                                    <span class="mr-16">排序: {{child.order}}</span>
                                    <a @click="getAdminMenuDetail(child)" class="edit-icon mr-12" href="javascript:void(0)"><i class="el-icon-edit"></i></a>
                                    <a @click="deleteAdminMenu(child)" title="删除" class="delete-icon" href="javascript:void(0)"><i class="el-icon-delete"></i></a>
                                </span>
                            </span>
                        </li>
                    </ul>
            ...
            <div>
                <el-dialog v-model="createDialogVisible" title="添加" @close="createDialogClosed" width="555px" destroy-on-close :close-on-click-modal="false">
                    <div>
...
<script>
    import { main, updateChildRef, createChildRef} from '@/hooks/adminPermissions/menu/index.js'
    import Create from '@/views/adminPermissions/menu/Create'
    import Update from '@/views/adminPermissions/menu/Update'
    export default {
        name: "AdminMenu",
        components: {
            Create,
